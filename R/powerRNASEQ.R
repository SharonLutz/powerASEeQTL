powerRNASEQ <-
  function(n, mu, fold, phi, theta, n.simu, alpha, maf, methods, sim, propASE)
  {
    ...
    pvalLin = pvalNB = pvalQP =  pvalTreCASE = rep(1, n.simu)
    pvalBB = rep(0, n.simu)
    ...
    for(k in 1:n.simu){
      ...
      # ------------------------------------------------
      # simulate data scenario 2: total read count
      # ------------------------------------------------
      if(sim=="sim2"){
      ...
      if(length(ynA)>0){
        # beta-binomial
        matAA<-cbind(y1A,ynA)
        modelBB<-summary(betabin(matAA),level=(1-alpha))
        
        if(!is.na(modelBB$coefficients)[1,3]){
          if(modelBB$coef[1,3]>0.5 | modelBB$coef[1,4]<0.5){pvalBB[k] <-1}
        }else{pvalBB[k] <- NA}
        
      }else{pvalBB[k]<-NA}
      # ------------------------------------------------
      # methods for both ASE eQTL
      # ------------------------------------------------
    ...
    # Return in this order: linear, negbin, quasi poisson, aseBB, trecase
    c(ppLin, ppNB, ppQP, ppvalBB, ppTreCASE)
